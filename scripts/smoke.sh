...
if [[ -z "$CLIENT_A_ID" || -z "$CLIENT_B_ID" ]]; then
  echo "FAIL: CLIENT_A_ID and CLIENT_B_ID must be set (UUIDs for Tenant A and Tenant B)." >&2
  exit 1
fi
...
login() {
  ...
  local client_id="$2"
  local payload
  payload=$(printf '{"email":"%s","password":"%s","client_id":"%s"}' "$email" "Passw0rd!" "$client_id")
  local result
...
admin_a_token=$(login "adminA@test.com" "$CLIENT_A_ID")
...
A1_ID=$(echo "$companies_admin_a" | json_get 'next(c["id"] for c in data["companies"] if c["name"] == "A1")')

viewer_a_token=$(login "viewerA@test.com" "$CLIENT_A_ID")
...
admin_b_token=$(login "adminB@test.com" "$CLIENT_B_ID")
