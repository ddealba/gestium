(function () {
  ...
  const loadDashboard = async () => {
    try {
      const data = await window.apiFetch('/dashboard/summary?days=14&overdue_limit=8&activity_limit=12');
      renderKpis(data?.kpis || {});
      renderDonut(data?.cases_by_status?.labels || [], data?.cases_by_status?.values || []);
      renderBar(data?.docs_by_status?.labels || [], data?.docs_by_status?.values || []);
